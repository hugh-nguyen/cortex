...
for app_stack_directory in os.listdir(log_dir):
    ...
    # latest_app_stacks.append(f"{app_stack_directory}/{sorted(app_stack_files)[-1]}")
...
for app_stack in app_stacks:
    ...
    transform = lambda d: {**d, "service": f"{d['service']}-{d['version'].replace('.', '-')}"}
    content = {
        f"{service_name}-{data['version'].replace('.', '-')}": {
            "version": data["version"],
            "depends_on": [transform(d) for d in data["depends_on"]]
        }
        for service_name, data in yaml.safe_load(open(app_stack_path, "r").read()).items()
    }
