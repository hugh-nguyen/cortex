...
func handleAddRoute(w http.ResponseWriter, r *http.Request) {
    ...
    routes[data.AppName] = &route.RouteConfiguration{
        Name: data.AppName,
    ...
    for _, rt := range data.Routes {
        ...
        routes[data.AppName].VirtualHosts[0].Routes = append(
            routes[data.AppName].VirtualHosts[0].Routes,
            makeRoute(
                rt.Prefix,
                rt.Cluster,
                rt.Headers,
    ...
    listeners[data.AppName] = makeListener(data.AppName, "0.0.0.0", 8080, data.AppName)
    ...
    log.Printf("Added route configuration '%s' with %d routes", data.AppName, len(routes[data.AppName].VirtualHosts[0].Routes))
